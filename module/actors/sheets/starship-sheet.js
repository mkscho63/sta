...
export class STAStarshipSheet extends ActorSheet {
    ...
    getData() {
        ...
        if (data.data.systems.communications.value > 12) data.data.systems.communications.value = 12;
        if (data.data.systems.computers.value > 12) data.data.systems.computers.value = 12;
        if (data.data.systems.engines.value > 12) data.data.systems.engines.value = 12;
        if (data.data.systems.sensors.value > 12) data.data.systems.sensors.value = 12;
        if (data.data.systems.structure.value > 12) data.data.systems.structure.value = 12;
        if (data.data.systems.weapons.value > 12) data.data.systems.weapons.value = 12;
        if (data.data.departments.command.value > 5) data.data.departments.command.value = 5;
        if (data.data.departments.conn.value > 5) data.data.departments.conn.value = 5;
        if (data.data.departments.engineering.value > 5) data.data.departments.engineering.value = 5;
        if (data.data.departments.medicine.value > 5) data.data.departments.medicine.value = 5;
        if (data.data.departments.science.value > 5) data.data.departments.science.value = 5;
        if (data.data.departments.security.value > 5) data.data.departments.security.value = 5;

        // Checks if shields is larger than its max, if so, set to max. 
        if (data.data.shields.value > data.data.shields.max) data.data.shields.value = data.data.shields.max;

        //Ensure system and department values aren't lower than their minimums.
        if (data.data.systems.communications.value < 7) data.data.systems.communications.value = 7;
        if (data.data.systems.computers.value < 7) data.data.systems.computers.value = 7;
        if (data.data.systems.engines.value < 7) data.data.systems.engines.value = 7;
        if (data.data.systems.sensors.value < 7) data.data.systems.sensors.value = 7;
        if (data.data.systems.structure.value < 7) data.data.systems.structure.value = 7;
        if (data.data.systems.weapons.value < 7) data.data.systems.weapons.value = 7;
        if (data.data.departments.command.value < 0) data.data.departments.command.value = 0;
        if (data.data.departments.conn.value < 0) data.data.departments.conn.value = 0;
        if (data.data.departments.engineering.value < 0) data.data.departments.engineering.value = 0;
        if (data.data.departments.medicine.value < 0) data.data.departments.medicine.value = 0;
        if (data.data.departments.science.value < 0) data.data.departments.science.value = 0;
        if (data.data.departments.security.value < 0) data.data.departments.security.value = 0;

        // Checks if shields is below 0, if so - set it to 0.
        if (data.data.shields.value < 0) data.data.shields.value = 0;
        if (data.data.power.value < 0) data.data.power.value = 0;
        if (data.data.crew.value < 0) data.data.crew.value = 0;
    ...
    activateListeners(html) {
        ...
        staActor.staRenderTracks(html, null, null, null, shieldsTrackMax, powerTrackMax, crewTrackMax);
        ...
        html.find('.control .delete').click(ev => {
            const li = $(ev.currentTarget).parents(".entry");
            var r = confirm("Are you sure you want to delete " + li[0].getAttribute("data-item-value") + "?");
            if (r == true) {
        ...
        html.find('[id^="shields"]').click(ev => {
            ...
            var newTotal = newTotalObject.id.substring("shields-".length);
            ...
            if (newTotalObject.getAttribute("data-selected") === "true") {
                ...
                var nextCheck = 'shields-' + (parseInt(newTotal) + 1);
                ...
                // If it isn't caught by the if, the next box is likely activated. If something happened, its safer to set the value anyway.
                    ...
                    var total = html.find('#total-shields')[0].value;
                    if (total != newTotal) {
        ...
        html.find('[id^="power"]').click(ev => {
            var newTotalObject = $(ev.currentTarget)[0];
            var newTotal = newTotalObject.id.substring("power-".length);
            // data-selected stores whether the track box is currently activated or not. This checks that the box is activated
            if (newTotalObject.getAttribute("data-selected") === "true") {
                // Now we check that the "next" track box is not activated. 
                // If there isn't one, or it isn't activated, we only want to decrease the value by 1 rather than setting the value.
                var nextCheck = 'power-' + (parseInt(newTotal) + 1);
                if (!html.find('#'+nextCheck)[0] || html.find('#'+nextCheck)[0].getAttribute("data-selected") != "true") {
                ...
                // If it isn't caught by the if, the next box is likely activated. If something happened, its safer to set the value anyway.
                    ...
                    var total = html.find('#total-power')[0].value;
                    if (total != newTotal) {
        ...
        html.find('[id^="crew"]').click(ev => {
            var newTotalObject = $(ev.currentTarget)[0];
            var newTotal = newTotalObject.id.substring("crew-".length);
            // data-selected stores whether the track box is currently activated or not. This checks that the box is activated
            if (newTotalObject.getAttribute("data-selected") === "true") {
                // Now we check that the "next" track box is not activated. 
                // If there isn't one, or it isn't activated, we only want to decrease the value by 1 rather than setting the value.
                var nextCheck = 'crew-' + (parseInt(newTotal) + 1);
                if (!html.find('#'+nextCheck)[0] || html.find('#'+nextCheck)[0].getAttribute("data-selected") != "true") {
                ...
                // If it isn't caught by the if, the next box is likely activated. If something happened, its safer to set the value anyway.
                    ...
                    var total = html.find('#total-crew')[0].value;
                    if (total != newTotal) {
